      ...
      <div className="max-h-56 overflow-y-auto rounded-xl border border-slate-200/50 bg-white/50">
        ...
        {users.map((u) => (
          <div
            ...
            className="flex items-center gap-3 p-3 cursor-pointer hover:bg-blue-50/50 transition-all duration-200 border-b border-slate-100/50 last:border-0 group"
          >
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${getAvatarColor(u.username)} flex items-center justify-center text-white text-xs font-bold shadow-sm group-hover:shadow-md group-hover:scale-105 transition-all duration-300`}>
              {getInitials(u.full_name || u.username)}
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm text-slate-700 truncate">{u.full_name || u.username}</p>
              <p className="text-xs text-slate-400 truncate">{u.email}</p>
            </div>
            <button className="px-3 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-100 rounded-lg transition-all duration-200">
              Add
            </button>
          </div>
        ))}
